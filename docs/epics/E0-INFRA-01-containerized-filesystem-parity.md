...
This epic is governed by `GPT.md` at the repository root. All implementation must comply with:
...
- `Co-Authored-By:` headers referencing AI tools (GPT, Copilot, etc.) in commit messages
...
**Rationale for Read-Only:** The web frontend is a read-only consumer of GPT's filesystem. Write operations occur exclusively through the Python runner system. Read-only mounts prevent accidental modification during development and match production semantics.
...
| 7   | Network `gpthome-net` defined with bridge driver                       | Code review                                     |
...
| 4   | `docker exec` into running container confirms `/thoughts` exists at root | `docker exec gpthome-web ls /thoughts` returns files                                    |
...
| 6   | Container process runs as non-root user                                          | `docker exec gpthome-web ps aux` shows non-root |
...
| 2   | Container runs `pnpm dev --turbopack` (not production build)                | `docker exec gpthome-web ps aux` shows `next dev` |
| 3   | Modifying `apps/web/src/app/page.tsx` on host triggers HMR within 3 seconds | Manual test with timestamp                        |
| 4   | No `ENOENT` or filesystem watching errors in container logs                 | `docker logs gpthome-web` clean                   |
...
{
  "scripts": {
    "docker:verify": "docker exec gpthome-web node /app/scripts/verify-fs-access.mjs"
  }
}
```
...
- [ ] PR approved by reviewer following No-AI policy scan (per GPT.md Section 1)
...
_This epic is governed by GPT.md. Any conflicts with other documentation are resolved in favor of GPT.md._
